...
CPPFLAGS += -Wa,-adhlns=$(<:%.cpp=$(BUILDDIR)/%.lst)
...
ASFLAGS = $(ADEFS) -Wa,-adhlns=$(<:%.S=$(BUILDDIR)/%.lst),-gstabs,--listing-cont-lines=100
...
OBJ = $(SRC:%.c=$(BUILDDIR)/%.o) $(CPPSRC:%.cpp=$(BUILDDIR)/%.o) $(ASRC:%.S=$(BUILDDIR)/%.o)

# Define all listing files.
LST = $(SRC:%.c=$(BUILDDIR)/%.lst) $(CPPSRC:%.cpp=$(BUILDDIR)/%.lst) $(ASRC:%.S=$(BUILDDIR)/%.lst)
...
clean_list :
	...
	$(REMOVE) $(SRC:%.c=$(BUILDDIR)/%.o)
	$(REMOVE) $(SRC:%.c=$(BUILDDIR)/%.lst)
	$(REMOVE) $(SRC:.c=.s)
	$(REMOVE) $(SRC:.c=.d)
	$(REMOVE) $(SRC:.c=.i)
	$(REMOVEDIR) .dep
